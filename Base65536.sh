...
if [ -f "$AI_Name".cpp ]; then #If the file exists
	...
	"$Compiler" "$AI_Name"_nopragma.cpp -o "$AI_Name" -Ofast -march=x86-64-v3 -std=c++17 -w -s -flto -fno-rtti -fno-math-errno -ffunction-sections -fdata-sections -Wl,--gc-sections -Wl,-z,norelro -Wl,--hash-style=gnu #-fno-exceptions
